  ...
  return (
    <div>
      <div className='text-3xl border-2 border-black flex flex-row gap-4 justify-center items-center p-4' style={{ fontFamily: "var(--font-story-script)" }}>    
        <a
          href={`https://connect.stripe.com/oauth/authorize?response_type=code&client_id=${process.env.NEXT_PUBLIC_STRIPE_CLIENT_ID}&scope=read_write`}
        ...
        <a
  href={`https://accounts.google.com/o/oauth2/v2/auth?client_id=${process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID}&redirect_uri=${encodeURIComponent(process.env.NEXT_PUBLIC_GOOGLE_REDIRECT_URI)}&response_type=code&scope=https://www.googleapis.com/auth/analytics.readonly&access_type=offline&prompt=consent&state=${user.id}`}
  className="btn bg-blue-500 text-white p-4 rounded border-4 border-black"
>
