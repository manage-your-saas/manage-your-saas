...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (data?.user) router.replace("/dashboard/seo");
    })();
    return () => {
      cancelled = true;
    };
              ...
              <div className={`mb-6 p-4 rounded-lg border ${
                ...
                <div className="flex items-start gap-3">
                  <svg className={`w-5 h-5 flex-shrink-0 mt-0.5 ${
                    msg.includes("not configured") ? "text-orange-600" : "text-red-600"
                  }`} fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                  </svg>
                  <div className="flex-1">
                    <p className={`text-sm font-medium ${
                      msg.includes("not configured") ? "text-orange-800" : "text-red-800"
                    }`}>{msg}</p>
                    {msg.includes("not configured") && (
                      <div className="mt-3 text-xs text-orange-700 space-y-1">
                        <p className="font-semibold">To enable Google login:</p>
